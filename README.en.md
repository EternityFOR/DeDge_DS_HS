...
> The current version is `0.1.82`. Release artifacts are primarily verified on Windows 10/11 x64. DeepSeek Harness remains a developer preview, so review the changelog and known limitations before upgrading.
...
`0.1.5-rc.1` is the bundled upstream DeepSeek Harness version; it is separate from the extension version `0.1.82`.
