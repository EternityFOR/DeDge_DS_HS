...
> The current version is `0.1.54`. Release artifacts are primarily verified on Windows 10/11 x64. DeepSeek Harness remains a developer preview, so review the changelog and known limitations before upgrading.
...
```powershell
code --install-extension .\dedge-deepseek-harness-vscode-0.1.54-win32-x64.vsix
```
...
`0.1.1-rc.1` is the bundled upstream DeepSeek Harness version; it is separate from the extension version `0.1.54`.
