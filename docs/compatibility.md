...
| 组件 | 内置版本 | 规则 |
| --- | --- | --- |
| DeepSeek Harness | `0.1.3-alpha.2` | 新版 journal/session 协议、原生图片附件与当前 Gateway 协议验证目标 |
| Node.js | `22.22.3` | VSIX 内置，不依赖系统 Node |
| pnpm | `11.21.0` | 只供 Harness 内部工具链使用 |

external JavaScript runtime 接受 Node `22.19+` 或 `24+`；Node 23 不在当前上游支持范围。bundled 和 external DSH 均要求精确的 `0.1.3-alpha.2`，版本不同会阻止启动，避免 API、认证和事件格式不一致。扩展不会从 PATH 猜测版本，也不会自动升级。
