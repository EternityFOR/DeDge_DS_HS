...
| 组件 | 内置版本 | 规则 |
| --- | --- | --- |
| DeepSeek Harness | `0.1.0-rc.6` | 必须精确匹配 |
| Node.js | `22.22.3` | VSIX 内置，不依赖系统 Node |
| pnpm | `11.21.0` | 只供 Harness 内部工具链使用 |

external JavaScript runtime 接受 Node `22.19+` 或 `24+`；Node 23 不在当前上游支持范围。external DSH 仍必须精确报告 `0.1.0-rc.6`。扩展不会从 PATH 猜测版本，也不会自动升级。
