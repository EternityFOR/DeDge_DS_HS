...
> 当前版本为 `0.1.35`。发布产物优先验证 Windows 10/11 x64；DeepSeek Harness 上游仍处于 developer preview，升级前请阅读变更日志和已知限制。
...
1. 从 [GitHub Releases](https://github.com/EternityFOR/DeDge_DS_HS/releases) 下载与 extension host 匹配的文件，例如 `dedge-deepseek-harness-vscode-0.1.35-win32-x64.vsix`。
...
```powershell
code --install-extension .\dedge-deepseek-harness-vscode-0.1.35-win32-x64.vsix
```
...
| --- | --- | --- |
| Windows 10/11 x64 | `win32-x64` | `0.1.35` 完整验证目标 |
